...
void GY_85::SetAccelerometer()
{
    ...
    Wire.write( 0x31 );                     // send register address      //DATA_FORMAT R
...
int* GY_85::readFromAccelerometer()
{
    ...
    //Serial.println(axis[0]);Serial.println(axis[1]);Serial.println(axis[2]);

  //  return axis[0],axis[1],axis[2];//new
    return axis;
}
//----------------------------------------
...
//R/W H_RESET SLEEP STBY_XG STBY_YG STBY_ZG CLK_SEL
void GY_85::SetGyro()
{
    ...
    Wire.beginTransmission( ITG3200 );
    Wire.write( 0x16 );//DLPF_FS
    Wire.write( 0x1E );                         // +/- 2000 dgrs/sec, 1KHz, 1E, 19
    Wire.endTransmission();

    Wire.beginTransmission( ITG3200 );
    Wire.write( 0x17 );//INT_CFG
    Wire.write( 0x00 );
    Wire.endTransmission();
